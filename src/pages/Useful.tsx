...
const Useful: React.FC = () => {
  ...
  const filteredArticles = useMemo(() => {
    return usefulArticles.filter(article => {
      const matchesCategory = activeCategory === "all" || article.category === activeCategory;
      const matchesQuery = !normalizedQuery || article.title.toLocaleLowerCase("uk-UA").includes(normalizedQuery);
  ...
  return (
    <div className="bg-white text-stone-800">
      <Helmet>
        ...
        />
        <link rel="canonical" href="https://irina-design.vercel.app/useful" />
        <meta property="og:url" content="https://irina-design.vercel.app/useful" />
        ...
        <Reveal as="section" className="section-padding bg-white">
          <div className="max-w-7xl mx-auto">
            <div className="flex items-end justify-between gap-6 mb-10">
              <div>
                <p className="text-xs uppercase tracking-[0.24em] text-stone-500 mb-3">Почати з цього</p>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-5">
              {featured.map(article => (
                ...
                >
                  ...
                  <div className="mt-auto flex items-center gap-2 text-sm text-stone-400">
                    Готується до публікації <FiArrowRight size={15} />
                  </div>
                </article>
              ))}
            </div>
          ...
          {query || activeCategory !== "all" ? (
            <>
              <div className="mb-8 flex flex-col md:flex-row md:items-end md:justify-between gap-3">
                <div>
                  <p className="text-xs uppercase tracking-[0.24em] text-stone-500 mb-3">Результат пошуку</p>
                  <h2 className="text-3xl md:text-4xl font-serif font-semibold text-stone-800">
              ...
              {filteredArticles.length > 0 ? (
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                  {filteredArticles.map(article => (
                    <article
                      key={article.id}
                      className="rounded-xl border border-stone-200 bg-white p-6 min-h-[160px] flex flex-col shadow-sm"
                    >
                      <span className="text-xs text-stone-400 mb-3">{String(article.id).padStart(2, "0")}</span>
                      <h3 className="text-lg font-serif font-semibold leading-snug text-stone-800">
                        {article.title}
                      </h3>
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-[300px_1fr] gap-8 lg:gap-12">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-px bg-stone-200 border border-stone-200 rounded-2xl overflow-hidden">
                        {articles.map(article => (
                          <article
                            key={article.id}
                            className="bg-white p-6 md:p-7 min-h-[150px] flex gap-5"
                          >
                            <span className="shrink-0 text-xs text-stone-400 pt-1">
                              {String(article.id).padStart(2, "0")}
                            </span>
                            <div className="flex flex-col">
                              <h3 className="text-lg md:text-xl font-serif font-semibold leading-snug text-stone-800">
                                {article.title}
                              </h3>
                              <span className="mt-auto pt-5 text-xs uppercase tracking-wider text-stone-400">
      ...
      <section className="bg-stone-900 text-white px-6 md:px-12 lg:px-24 py-20 md:py-24">
        <div className="max-w-4xl mx-auto text-center">
          <p className="text-xs uppercase tracking-[0.28em] text-stone-400 mb-4">Потрібна відповідь саме для вашого простору?</p>
